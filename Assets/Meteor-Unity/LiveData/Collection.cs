...
using ICollection = Meteor.Internal.ICollection;
...
namespace Meteor
{
        ...
        where TRecordType : MongoDocument, new()
    {
        ...
        public Collection(string name) : base()
        {
            ...
            var isCollectionTemporary = doesCollectionAlreadyExist &&
                                        LiveData.Instance.Collections[name] as TemporaryCollection != null;
            if (!isNameEmpty
        ...
        {
            selector = selector ?? delegate(TRecordType arg) { return true; };
        ...
        void Meteor.Internal.ICollection.Added(string messageText)
        {
            ...
            var r = message.fields ?? new TRecordType();
            r._id = message.id;
